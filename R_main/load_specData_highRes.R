...
Y_pred=predict_spectrolyzer(
  ...
  variable = var_name,
  #trans="none",
  trans=filter(eval$eval,variable==var_name)%>%filter(rmse==min(rmse))%>%pull(trans),
  #set="spc"
  set=filter(eval$eval,variable==var_name)%>%filter(rmse==min(rmse))%>%pull(set)
)
...
ggplot(all_mine_clean,
  ...
  geom_line(data=spc_data,aes(x=date,y=spc$`250.0`*X_scaling),linewidth=.1,col="red4")+
